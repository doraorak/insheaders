//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGDirectActionLogMessageSectionController_h
#define IGDirectActionLogMessageSectionController_h
@import Foundation;

#include "IGDirectMessageDisplayAnnouncementSectionController.h"
#include "IGCoreTextLinkHandler-Protocol.h"
#include "IGCoreTextNonLinkHandler-Protocol.h"
#include "IGDirectMessageSectionComponents.h"
#include "IGDirectMessageViewModelProtocol-Protocol.h"
#include "IGStoryViewerPresenting-Protocol.h"
#include "UIGestureRecognizerDelegate-Protocol.h"

@class IGUser, NSString;

@interface IGDirectActionLogMessageSectionController : IGDirectMessageDisplayAnnouncementSectionController<IGCoreTextLinkHandler, IGCoreTextNonLinkHandler, UIGestureRecognizerDelegate> {
  /* instance variables */
  IGDirectMessageSectionComponents *_components;
  NSObject<IGStoryViewerPresenting> *_controller;
  IGUser *_otherUser;
  NSObject<IGDirectMessageViewModelProtocol> *_viewModel;
}

@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@optional
@property (readonly, copy) NSString *debugDescription;

/* instance methods */
- (id)initWithComponents:(id)components;
- (struct CGSize { double x0; double x1; })sizeForViewModel:(id)model;
- (Class)cellClass;
- (void)configureCell:(id)cell withViewModel:(id)model;
- (void)coreTextView:(id)view didTapOnString:(id)string URL:(id)url;
- (void)coreTextView:(id)view didLongTapOnString:(id)string URL:(id)url;
- (void)coreTextViewDidTapOnNonLinkedString:(id)string touchEvent:(unsigned long long)event;
- (void)coreTextViewDidLongTapOnNonLinkedString:(id)string atPoint:(struct CGPoint { double x0; double x1; })point touchEvent:(unsigned long long)event;
- (void)listAdapter:(id)adapter willDisplaySectionController:(id)controller cell:(id)cell atIndex:(long long)index;
- (id)viewModel;
- (void)setViewModel:(id)model;
@end

#endif /* IGDirectActionLogMessageSectionController_h */
