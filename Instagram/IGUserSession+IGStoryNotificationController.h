//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGUserSession_IGStoryNotificationController_h
#define IGUserSession_IGStoryNotificationController_h
@import Foundation;

#include "IGStoryDataControllerStoryItemListener-Protocol.h"
#include "IGStoryViewerDelegate-Protocol.h"
#include "UIViewControllerTransitioningDelegate-Protocol.h"

@class IGMainAppSurfaceIntent, IGUserSession, NSArray, NSMapTable, NSString;

@interface IGUserSession (IGStoryNotificationController)
/* instance methods */
- (id)storyNotificationController;
@end

#endif /* IGUserSession_IGStoryNotificationController_h */
