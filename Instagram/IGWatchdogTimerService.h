//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGWatchdogTimerService_h
#define IGWatchdogTimerService_h
@import Foundation;

@class FBWatchdogTimer, IGWatchdogConfig, IGWatchdogExpiryHandler;

@interface IGWatchdogTimerService : NSObject {
  /* instance variables */
  BOOL _appWasActiveOnce;
  IGWatchdogConfig *_watchdogTimerConfig;
  FBWatchdogTimer *_watchdogTimer;
  IGWatchdogExpiryHandler *_watchdogExpiryHandler;
}

/* instance methods */
- (id)initWithWatchdogTimer:(id)timer watchdogExpiryHandler:(id)handler watchdogConfig:(id)config;
@end

#endif /* IGWatchdogTimerService_h */
