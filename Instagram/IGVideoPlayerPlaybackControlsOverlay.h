//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGVideoPlayerPlaybackControlsOverlay_h
#define IGVideoPlayerPlaybackControlsOverlay_h
@import Foundation;

#include "IGVideoPlayerOverlay.h"
#include "IGModernVideoControlsView.h"
#include "IGUnifiedVideoControlsViewDelegate-Protocol.h"
#include "IGVideoPreviewThumbnailView.h"

@class IGGradientView, NSString, UIView;
@protocol IGVideoPlayerPlaybackControlsOverlayDelegate;

@interface IGVideoPlayerPlaybackControlsOverlay : IGVideoPlayerOverlay<IGUnifiedVideoControlsViewDelegate> {
  /* instance variables */
  UIView *_containerView;
  IGModernVideoControlsView *_playbackControlsView;
  IGVideoPreviewThumbnailView *_videoPreviewThumbnailView;
  UIView *_separatorLine;
  UIView *_legibilityContainerView;
  IGGradientView *_legibilityGradient;
  BOOL _isScrubbing;
  BOOL _shouldHideScrubberOnNavigation;
}

@property (weak, nonatomic) NSObject<IGVideoPlayerPlaybackControlsOverlayDelegate> *delegate;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@optional
@property (readonly, copy) NSString *debugDescription;

/* instance methods */
- (id)initWithStatusAdapter:(id)adapter launcherSet:(id)set;
- (void)loadView;
- (struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })preferredFrameInsideRect:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })rect leftToolbarItemsFrame:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })frame rightToolbarItemsFrame:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })frame;
- (void)layoutOverlayViews;
- (void)reset;
- (void)overlayContextDidRegisterSingleTap:(id)tap;
- (void)overlayStatusAdapterDidLoadVideo:(id)video;
- (void)updateProgressIndicatorWithProgress:(double)progress remainingDuration:(double)duration elapsedDuration:(double)duration totalDuration:(double)duration;
- (void)updateScrubberTimestampVisibility:(BOOL)visibility;
- (void)videoControlsViewProgressScrubbingDidBegin:(id)begin;
- (void)videoControlsViewProgressScrubbingDidChange:(id)change;
- (void)videoControlsViewProgressScrubbingDidEnd:(id)end;
- (void)videoControlsView:(id)view didRecognizeTapGesture:(id)gesture;
- (void)videoControlsViewAutoAdvanceModeCancelButtonTapped:(id)tapped;
@end

#endif /* IGVideoPlayerPlaybackControlsOverlay_h */
