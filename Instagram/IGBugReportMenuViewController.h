//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGBugReportMenuViewController_h
#define IGBugReportMenuViewController_h
@import Foundation;

#include "IGViewController.h"
#include "IGBugReportEventLogger.h"
#include "IGBugReportingMenuDelegate-Protocol.h"
#include "IGBugReportingMenuViewControllerProtocol-Protocol.h"
#include "IGGestureHandler-Protocol.h"
#include "IGPartialModalSheetListener-Protocol.h"
#include "UITableViewDataSource-Protocol.h"
#include "UITableViewDelegate-Protocol.h"

@class IGDeviceSession, IGUserSession, NSArray, NSString, UITableView;

@interface IGBugReportMenuViewController : IGViewController<IGGestureHandler, IGPartialModalSheetListener, UITableViewDataSource, UITableViewDelegate, IGBugReportingMenuViewControllerProtocol> {
  /* instance variables */
  IGDeviceSession *_deviceSession;
  IGUserSession *_userSession;
  long long _style;
  IGBugReportEventLogger *_logger;
  NSArray *_pinnedActions;
  UITableView *_tableView;
  long long _internalSettingsAvailabilityStatus;
}

@property (weak, nonatomic) NSObject<IGBugReportingMenuDelegate> *delegate;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@optional
@property (readonly, copy) NSString *debugDescription;
@property (readonly) NSString *navChain;
@property (readonly) NSString *endpoint;

/* instance methods */
- (id)initWithDeviceSession:(id)session userSession:(id)session style:(long long)style navChain:(id)chain endpoint:(id)endpoint;
- (void)loadView;
- (void)viewWillAppear:(BOOL)appear;
- (void)viewDidAppear:(BOOL)appear;
- (void)viewDidLayoutSubviews;
- (BOOL)prefersNavigationBarHidden;
- (long long)numberOfSectionsInTableView:(id)view;
- (long long)tableView:(id)view numberOfRowsInSection:(long long)section;
- (id)tableView:(id)view cellForRowAtIndexPath:(id)path;
- (double)tableView:(id)view heightForRowAtIndexPath:(id)path;
- (BOOL)tableView:(id)view shouldHighlightRowAtIndexPath:(id)path;
- (void)tableView:(id)view didSelectRowAtIndexPath:(id)path;
- (void)bugReportingActionCellButtonTapped:(id)tapped;
- (void)bugReportingLinkActionCellButtonTapped:(id)tapped;
- (void)switchProvider:(id)provider switchToggled:(BOOL)toggled;
- (void)partialModalSheet:(id)sheet didChangeState:(unsigned long long)state;
- (void)partialModalSheet:(id)sheet didUpdateOffset:(double)offset presentationProgress:(double)progress;
- (void)partialModalSheet:(id)sheet willChangeState:(unsigned long long)state;
- (void)partialModalSheetDidDismiss:(id)dismiss;
- (void)partialModalSheet:(id)sheet didTapDimmingBackgroundView:(id)view atPoint:(struct CGPoint { double x0; double x1; })point timeSinceOpening:(double)opening;
- (BOOL)canRespondToGesture:(id)gesture;
@end

#endif /* IGBugReportMenuViewController_h */
