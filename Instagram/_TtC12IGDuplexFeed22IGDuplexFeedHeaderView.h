//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef _TtC12IGDuplexFeed22IGDuplexFeedHeaderView_h
#define _TtC12IGDuplexFeed22IGDuplexFeedHeaderView_h
@import Foundation;

#include "IGBaseView.h"
#include "IGStoryAnimatableDestinationView-Protocol.h"

@interface _TtC12IGDuplexFeed22IGDuplexFeedHeaderView : IGBaseView<IGStoryAnimatableDestinationView> { // (Swift)
  /* instance variables */
   reelsButton;
   followingButton;
   cameraButton;
   messagingButton;
   legibilityGradient;
   backgroundView;
   selectedItem;
   displayMode;
   backgroundRegion;
   delegate;
}

/* instance methods */
- (struct CGSize { double x0; double x1; })sizeThatFits:(struct CGSize { double x0; double x1; })fits;
- (void)layoutSubviews;
- (BOOL)canPerformAnimation;
- (id)contentViewForTransitionAnimation;
- (void)animateVisualMessageSendWithDuration:(double)duration delay:(double)delay style:(long long)style ringViewSpecFactory:(id)factory completion:(id /* block */)completion;
- (void)animateVisualMessageSendWithDuration:(double)duration delay:(double)delay ringViewSpec:(id)spec completion:(id /* block */)completion;
- (void)reelsButtonSelected;
- (void)followingButtonSelected;
- (void)messagingButtonSelected;
- (void)cameraButtonSelected;
- (id)initWithFrame:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })frame;
@end

#endif /* _TtC12IGDuplexFeed22IGDuplexFeedHeaderView_h */
