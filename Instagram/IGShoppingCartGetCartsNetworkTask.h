//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGShoppingCartGetCartsNetworkTask_h
#define IGShoppingCartGetCartsNetworkTask_h
@import Foundation;

#include "IGAPIClient-Protocol.h"
#include "IGShoppingCartNetworkTaskState.h"
#include "NSObject-Protocol.h"

@class IGObjectStores, NSSet, NSString;

@interface IGShoppingCartGetCartsNetworkTask : NSObject<NSObject> {
  /* instance variables */
  NSObject<IGAPIClient> *_networker;
  IGObjectStores *_objectStores;
  NSSet *_merchantPks;
}

@property (readonly, copy, nonatomic) IGShoppingCartNetworkTaskState *state;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@optional
@property (readonly, copy) NSString *debugDescription;

/* instance methods */
- (id)initWithNetworker:(id)networker objectStores:(id)stores merchantPks:(id)pks;
- (id)queries;
- (id)affectedMerchantBagPks;
- (id)affectedMutationIds;
- (BOOL)executeTaskWithSuccessHandler:(id /* block */)handler failureHandler:(id /* block */)handler;
@end

#endif /* IGShoppingCartGetCartsNetworkTask_h */
