//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef BKBloksCollectionPrecomputedLayoutResult_h
#define BKBloksCollectionPrecomputedLayoutResult_h
@import Foundation;

@class BKModel;
@protocol {RCLayoutResult="layout"{RCLayout="component"@"<RCMountable>""size"{CGSize="width"d"height"d}"children"{shared_ptr<const std::vector<RCLayoutChild>>="__ptr_"^v"__cntrl_"^{__shared_weak_count}}"extra"@"NSDictionary"}"cache"{shared_ptr<const RCLayoutCache>="__ptr_"^{RCLayoutCache}"__cntrl_"^{__shared_weak_count}}};

@interface BKBloksCollectionPrecomputedLayoutResult : NSObject {
  /* instance variables */
  struct RCLayoutResult { struct RCLayout { <RCMountable> *component; struct CGSize { double width; double height; } size; struct shared_ptr<const std::vector<RCLayoutChild>> { void *__ptr_; struct __shared_weak_count *__cntrl_; } children; NSDictionary *extra; } layout; struct shared_ptr<const RCLayoutCache> { struct RCLayoutCache *__ptr_; struct __shared_weak_count *__cntrl_; } cache; } _layoutResult;
  struct RCSizeRange { struct CGSize { double width; double height; } min; struct CGSize { double width; double height; } max; } _constrainingSize;
}

@property (readonly, nonatomic) BKModel *model;

/* instance methods */
- (const struct RCSizeRange *)constrainingSize;
@end

#endif /* BKBloksCollectionPrecomputedLayoutResult_h */
