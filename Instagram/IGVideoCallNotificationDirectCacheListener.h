//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGVideoCallNotificationDirectCacheListener_h
#define IGVideoCallNotificationDirectCacheListener_h
@import Foundation;

#include "IGAnalyticsEventLoggingProtocol-Protocol.h"
#include "IGDirectCacheListener-Protocol.h"

@class IGRealtimeMqttClient, IGUserSessionProvider, NSString;

@interface IGVideoCallNotificationDirectCacheListener : NSObject<IGDirectCacheListener> {
  /* instance variables */
  IGUserSessionProvider *_userSessionProvider;
  NSString *_userPk;
  IGRealtimeMqttClient *_realtimeMqttClient;
  NSObject<IGAnalyticsEventLoggingProtocol> *_analyticsLogger;
}

@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@optional
@property (readonly, copy) NSString *debugDescription;

/* instance methods */
- (id)initWithUserSessionProvider:(id)provider userPk:(id)pk directCache:(id)cache realtimeMqttClient:(id)client analyticsLogger:(id)logger;
- (void)directCacheDidUpdateThreads:(id)threads;
- (void)directCacheDidRemoveThreadsWithIds:(id)ids;
- (void)directCacheDidUpdateThread:(id)thread withAppliedUpdates:(id)updates;
- (void)directCacheDidUpdateInboxMetadata;
@end

#endif /* IGVideoCallNotificationDirectCacheListener_h */
