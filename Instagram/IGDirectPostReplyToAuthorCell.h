//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGDirectPostReplyToAuthorCell_h
#define IGDirectPostReplyToAuthorCell_h
@import Foundation;

#include "IGDirectMessageCell.h"
#include "IGDirectTextMessageBubbleView.h"
#include "IGVideoIndicatorView.h"

@class IGCoreTextView, IGDirectMessageBubbleView, IGDirectMessageReplyBarView, IGImageProgressView, UIImageView;

@interface IGDirectPostReplyToAuthorCell : IGDirectMessageCell {
  /* instance variables */
  struct CGSize { double width; double height; } _postContentSize;
  double _dottedLinePadding;
  IGCoreTextView *_titleView;
  IGDirectMessageBubbleView *_imageBubbleView;
  IGImageProgressView *_imageView;
  IGDirectTextMessageBubbleView *_textBubbleView;
  IGDirectMessageReplyBarView *_replyBarView;
  IGVideoIndicatorView *_videoIndicatorView;
  UIImageView *_sidecarIconView;
  UIImageView *_shoppingIconView;
}

/* class methods */
+ (BOOL)needsAttachedBubbleView;
+ (BOOL)needsContainerStyling;

/* instance methods */
- (id)initWithFrame:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })frame;
- (void)layoutSubviews;
- (struct CGSize { double x0; double x1; })messageContentSize;
@end

#endif /* IGDirectPostReplyToAuthorCell_h */
