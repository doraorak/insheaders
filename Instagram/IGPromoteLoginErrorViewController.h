//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGPromoteLoginErrorViewController_h
#define IGPromoteLoginErrorViewController_h
@import Foundation;

#include "IGPromoteViewController.h"
#include "IGBusinessErrorView.h"
#include "IGPromoteBusinessUserAccessTokenErrorModel.h"

@class IGUserSession;

@interface IGPromoteLoginErrorViewController : IGPromoteViewController {
  /* instance variables */
  IGUserSession *_userSession;
  IGPromoteBusinessUserAccessTokenErrorModel *_errorModel;
  IGBusinessErrorView *_errorView;
}

/* instance methods */
- (id)initWithErrorModel:(id)model userSession:(id)session;
- (void)viewDidLoad;
- (void)viewWillAppear:(BOOL)appear;
- (void)_onCancelBarButtonTapped;
@end

#endif /* IGPromoteLoginErrorViewController_h */
