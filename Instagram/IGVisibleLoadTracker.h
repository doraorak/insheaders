//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGVisibleLoadTracker_h
#define IGVisibleLoadTracker_h
@import Foundation;

#include "IGAnalyticsEventLoggingProtocol-Protocol.h"

@class NSString;

@interface IGVisibleLoadTracker : NSObject {
  /* instance variables */
  NSObject<IGAnalyticsEventLoggingProtocol> *_analyticsLogger;
  long long _mode;
  NSString *_analyticsModule;
  long long _currentIndex;
  double _visibleLoadStartTime;
}

/* instance methods */
- (id)initWithMode:(long long)mode analyticsModule:(id)module analyticsLogger:(id)logger;
- (id)viewpointAction;
@end

#endif /* IGVisibleLoadTracker_h */
