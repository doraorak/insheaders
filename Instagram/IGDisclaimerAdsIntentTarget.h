//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGDisclaimerAdsIntentTarget_h
#define IGDisclaimerAdsIntentTarget_h
@import Foundation;

#include "FBIntentTarget-Protocol.h"
#include "IGSponsoredInfoProviding-Protocol.h"

@class IGMedia, NSString;

@interface IGDisclaimerAdsIntentTarget : NSObject<FBIntentTarget>

@property (readonly, nonatomic) IGMedia *media;
@property (readonly, nonatomic) NSObject<IGSponsoredInfoProviding> *sponsoredInfoProvider;
@property (readonly, nonatomic) unsigned long long surfaceType;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@optional
@property (readonly, copy) NSString *debugDescription;

/* instance methods */
- (id)initWithMedia:(id)media sponsoredInfoProvider:(id)provider surfaceType:(unsigned long long)type;
@end

#endif /* IGDisclaimerAdsIntentTarget_h */
