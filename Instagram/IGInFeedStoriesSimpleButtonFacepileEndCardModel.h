//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGInFeedStoriesSimpleButtonFacepileEndCardModel_h
#define IGInFeedStoriesSimpleButtonFacepileEndCardModel_h
@import Foundation;

#include "IGInFeedStoriesSimpleButtonEndCardModel.h"

@class NSArray;

@interface IGInFeedStoriesSimpleButtonFacepileEndCardModel : IGInFeedStoriesSimpleButtonEndCardModel {
  /* instance variables */
  BOOL _isCloseFriends;
}

@property (readonly, copy, nonatomic) NSArray *users;

/* class methods */
+ (id)makeWithPK:(id)pk type:(long long)type filteringTag:(id)tag entryDictionary:(id)dictionary objectStores:(id)stores;

/* instance methods */
- (BOOL)isEqual:(id)equal;
@end

#endif /* IGInFeedStoriesSimpleButtonFacepileEndCardModel_h */
