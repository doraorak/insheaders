//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef _TtC11BCNComposer34BCNComposerTypeaheadViewController_h
#define _TtC11BCNComposer34BCNComposerTypeaheadViewController_h
@import Foundation;

#include "IGViewController.h"
#include "IGAutocompleteSectionControllerProvider-Protocol.h"
#include "IGUserAndHashtagAutocompleteViewControllerDelegate-Protocol.h"

@interface _TtC11BCNComposer34BCNComposerTypeaheadViewController : IGViewController<IGAutocompleteSectionControllerProvider, IGUserAndHashtagAutocompleteViewControllerDelegate> { // (Swift)
  /* instance variables */
   delegate;
   userSession;
   textInput;
   analyticsModule;
   enableFollowingGlyph;
   shadowView;
   containerView;
   blurView;
   blurOverlayView;
   $__lazy_storage_$_dismissButton;
   $__lazy_storage_$_autocompleteViewController;
   currentQuery;
   currentAutocompleteRange;
   currentAutoCompleteType;
}

/* instance methods */
- (void)viewDidLoad;
- (void)viewDidLayoutSubviews;
- (void)traitCollectionDidChange:(id)change;
- (void)didTapDismissButton:(id)button;
- (id)sectionControllerForObject:(id)object;
- (void)userAndHashtagAutocompleteViewController:(id)controller didSelectResultAtIndex:(long long)index autocompleteText:(id)text allResults:(id)results autocompleteType:(long long)type;
- (void)userAndHashtagAutocompleteViewController:(id)controller didLoadResults:(long long)results;
- (void)userAndHashtagAutocompleteViewController:(id)controller didUpdateContentSize:(struct CGSize { double x0; double x1; })size;
- (id)initWithAnalyticsModule:(id)module performanceListener:(id)listener;
@end

#endif /* _TtC11BCNComposer34BCNComposerTypeaheadViewController_h */
