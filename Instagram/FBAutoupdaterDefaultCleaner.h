//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef FBAutoupdaterDefaultCleaner_h
#define FBAutoupdaterDefaultCleaner_h
@import Foundation;

#include "FBAutoupdaterHost.h"
#include "NSObject-Protocol.h"

@class NSFileManager, NSString;
@protocol OS_dispatch_queue;

@interface FBAutoupdaterDefaultCleaner : NSObject<NSObject> {
  /* instance variables */
  NSFileManager *_fileManager;
  NSString *_trashPath;
  NSObject<OS_dispatch_queue> *_trashQueue;
}

@property (readonly, nonatomic) FBAutoupdaterHost *host;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@optional
@property (readonly, copy) NSString *debugDescription;

/* instance methods */
- (id)initWithHost:(id)host;
- (void)removePath:(id)path;
- (void)removeUnappliedUpdates;
- (void)removeTemporaryFiles;
- (void)cleanUpAfterUpdateInstallationAttempt;
@end

#endif /* FBAutoupdaterDefaultCleaner_h */
