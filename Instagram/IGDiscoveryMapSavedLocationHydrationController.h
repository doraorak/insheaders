//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGDiscoveryMapSavedLocationHydrationController_h
#define IGDiscoveryMapSavedLocationHydrationController_h
@import Foundation;

@class NSArray, NSMutableOrderedSet, NSMutableSet;
@protocol IGDiscoveryMapSavedLocationHydrationControllerDelegate;

@interface IGDiscoveryMapSavedLocationHydrationController : NSObject {
  /* instance variables */
  NSArray *_locationIDs;
  NSMutableOrderedSet *_pagesToFetchStack;
  NSMutableSet *_inFlightHydrationLocationPages;
  NSMutableSet *_completedHydrationLocationPages;
}

@property (weak, nonatomic) NSObject<IGDiscoveryMapSavedLocationHydrationControllerDelegate> *delegate;

/* instance methods */
- (id)initWithLocationIDs:(id)ids;
@end

#endif /* IGDiscoveryMapSavedLocationHydrationController_h */
