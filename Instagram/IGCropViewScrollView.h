//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGCropViewScrollView_h
#define IGCropViewScrollView_h
@import Foundation;

#include "UIScrollView.h"
#include "UIGestureRecognizerDelegate-Protocol.h"

@class NSMutableSet, NSString;

@interface IGCropViewScrollView : UIScrollView<UIGestureRecognizerDelegate> {
  /* instance variables */
  NSMutableSet *_blockedGestures;
}

@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@optional
@property (readonly, copy) NSString *debugDescription;

/* instance methods */
- (BOOL)gestureRecognizer:(id)recognizer shouldRecognizeSimultaneouslyWithGestureRecognizer:(id)recognizer;
- (BOOL)gestureRecognizerShouldBegin:(id)begin;
- (void)setContentSize:(struct CGSize { double x0; double x1; })size;
@end

#endif /* IGCropViewScrollView_h */
