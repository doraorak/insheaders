//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef NativeDevSettingsSpec_Protocol_h
#define NativeDevSettingsSpec_Protocol_h
@import Foundation;

@protocol NativeDevSettingsSpec <RCTBridgeModule, RCTTurboModule>
/* instance methods */
- (void)reload;
- (void)reloadWithReason:(id)reason;
- (void)onFastRefresh;
- (void)setHotLoadingEnabled:(BOOL)enabled;
- (void)setIsDebuggingRemotely:(BOOL)remotely;
- (void)setProfilingEnabled:(BOOL)enabled;
- (void)toggleElementInspector;
- (void)addMenuItem:(id)item;
- (void)addListener:(id)listener;
- (void)removeListeners:(double)listeners;
- (void)setIsShakeToShowDevMenuEnabled:(BOOL)enabled;
@end

#endif /* NativeDevSettingsSpec_Protocol_h */
