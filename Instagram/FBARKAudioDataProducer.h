//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef FBARKAudioDataProducer_h
#define FBARKAudioDataProducer_h
@import Foundation;

@class FBMPBaseProducerPort;
@protocol OS_dispatch_queue;

@interface FBARKAudioDataProducer : NSObject {
  /* instance variables */
  NSObject<OS_dispatch_queue> *_dataProductionQueue;
  double _lastProducedDataTimestamp;
  unsigned long long _dataCounter;
}

@property (readonly, nonatomic) FBMPBaseProducerPort *producerPort;

/* class methods */
+ (id)newWithProducerPort:(id)port;

/* instance methods */
- (id)initWithProducerPort:(id)port;
- (void)produceAudioDataWithAudioSampleBuffer:(struct opaqueCMSampleBuffer *)buffer;
@end

#endif /* FBARKAudioDataProducer_h */
