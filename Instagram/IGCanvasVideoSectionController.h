//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGCanvasVideoSectionController_h
#define IGCanvasVideoSectionController_h
@import Foundation;

#include "IGListSectionController.h"
#include "IGCanvasElementLogger.h"
#include "IGCanvasLoggingContext.h"
#include "IGCanvasVideoCell.h"
#include "IGCanvasVideoViewModel.h"
#include "IGListDisplayDelegate-Protocol.h"
#include "IGListScrollDelegate-Protocol.h"

@class IGCollectionViewVisibility, NSString;

@interface IGCanvasVideoSectionController : IGListSectionController<IGListDisplayDelegate, IGListScrollDelegate> {
  /* instance variables */
  IGCanvasVideoViewModel *_videoViewModel;
  IGCollectionViewVisibility *_collectionVisibility;
  IGCanvasElementLogger *_elementLogger;
  IGCanvasLoggingContext *_loggingContext;
  IGCanvasVideoCell *_currentVideoCell;
}

@property (nonatomic) BOOL audioEnabled;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@optional
@property (readonly, copy) NSString *debugDescription;

/* instance methods */
- (id)initWithCollectionView:(id)view loggingContext:(id)context;
- (void)seekToBeginning;
- (struct CGSize { double x0; double x1; })sizeForItemAtIndex:(long long)index;
- (id)cellForItemAtIndex:(long long)index;
- (void)didUpdateToObject:(id)object;
- (id)scrollDelegate;
- (id)displayDelegate;
- (void)listAdapter:(id)adapter willDisplaySectionController:(id)controller cell:(id)cell atIndex:(long long)index;
- (void)listAdapter:(id)adapter didEndDisplayingSectionController:(id)controller cell:(id)cell atIndex:(long long)index;
- (void)listAdapter:(id)adapter willDisplaySectionController:(id)controller;
- (void)listAdapter:(id)adapter didEndDisplayingSectionController:(id)controller;
- (void)canvasVideoCellDidLoadVideo:(id)video;
- (void)canvasVideoCellDidSeekToBeginning:(id)beginning;
- (void)listAdapter:(id)adapter didScrollSectionController:(id)controller;
- (void)listAdapter:(id)adapter willBeginDraggingSectionController:(id)controller;
- (void)listAdapter:(id)adapter didEndDraggingSectionController:(id)controller willDecelerate:(BOOL)decelerate;
- (double)componentViewabilityNumberOfTotalBlocks;
- (double)componentViewabilityNumberOfBlocksSeen:(double)seen;
@end

#endif /* IGCanvasVideoSectionController_h */
