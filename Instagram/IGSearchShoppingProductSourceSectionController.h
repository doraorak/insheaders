//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGSearchShoppingProductSourceSectionController_h
#define IGSearchShoppingProductSourceSectionController_h
@import Foundation;

#include "IGListSectionController.h"
#include "IGShoppingBrandSelectionViewControllerDelegate-Protocol.h"
#include "IGShoppingProductSourceSelectionConfiguration.h"

@class IGShoppingProductSourceModel, IGUserSession, NSString;

@interface IGSearchShoppingProductSourceSectionController : IGListSectionController {
  /* instance variables */
  IGShoppingProductSourceModel *_viewModel;
  IGUserSession *_userSession;
  NSString *_module;
  NSString *_taggingSessionId;
  long long _surfaceType;
  IGShoppingProductSourceSelectionConfiguration *_productSourceSelectionConfig;
  long long _taggingUserType;
  NSObject<IGShoppingBrandSelectionViewControllerDelegate> *_brandSelectionDelegate;
}

/* instance methods */
- (id)initWithUserSession:(id)session module:(id)module taggingSessionId:(id)id surfaceType:(long long)type productSourceSelectionConfig:(id)config;
- (long long)numberOfItems;
- (struct CGSize { double x0; double x1; })sizeForItemAtIndex:(long long)index;
- (id)cellForItemAtIndex:(long long)index;
- (void)didSelectItemAtIndex:(long long)index;
- (void)didUpdateToObject:(id)object;
@end

#endif /* IGSearchShoppingProductSourceSectionController_h */
