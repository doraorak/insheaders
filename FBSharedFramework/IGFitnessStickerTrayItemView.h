//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGFitnessStickerTrayItemView_h
#define IGFitnessStickerTrayItemView_h
@import Foundation;

#include "UIView.h"
#include "IGImageView.h"
#include "IGStickerViewType-Protocol.h"
#include "IGStoryStickerExportModel.h"

@class CAShapeLayer, NSString, UILabel, UIView;

@interface IGFitnessStickerTrayItemView : UIView<IGStickerViewType> {
  /* instance variables */
  CAShapeLayer *_backdropLayer;
  UIView *_imageContainerView;
  UILabel *_label;
  IGImageView *_imageView;
  double _imageOverflowOffset;
  double _redesignTextOffset;
  double _textRightInset;
  double _preferredFontSize;
  double _scale;
}

@property (readonly, nonatomic) IGStoryStickerExportModel *exportModel;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@optional
@property (readonly, copy) NSString *debugDescription;

/* class methods */
+ (id)newWithMetadata:(id)metadata resourceDirectory:(id)directory userSession:(id)session error:(id *)error;

/* instance methods */
- (id)initWithFrame:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })frame;
- (void)layoutSubviews;
- (id)stickerView;
- (id)tapModelsForCompositionSize:(struct CGSize { double x0; double x1; })size;
- (BOOL)isDraftSupported;
- (id)metadataForResourceDirectory:(id)directory userSession:(id)session error:(id *)error;
@end

#endif /* IGFitnessStickerTrayItemView_h */
