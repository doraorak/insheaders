//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGTVHackyCollectionView_h
#define IGTVHackyCollectionView_h
@import Foundation;

#include "IGGestureCoordinatingCollectionView.h"

@class NSHashTable;

@interface IGTVHackyCollectionView : IGGestureCoordinatingCollectionView {
  /* instance variables */
  BOOL _preventContentOffsetChangesFromSetContentSize;
  BOOL _preventUnderscrollContentOffsetChanges;
  NSHashTable *_registeredScrollViews;
}

/* instance methods */
- (id)initWithFrame:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })frame collectionViewLayout:(id)layout;
- (void)setContentOffset:(struct CGPoint { double x0; double x1; })offset;
- (void)setContentInset:(struct UIEdgeInsets { double x0; double x1; double x2; double x3; })inset;
- (void)setContentSize:(struct CGSize { double x0; double x1; })size;
- (BOOL)isScrollEnabled;
- (BOOL)touchesShouldCancelInContentView:(id)view;
@end

#endif /* IGTVHackyCollectionView_h */
