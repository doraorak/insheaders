//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef METAInAppPurchaseMockApplePaymentService_h
#define METAInAppPurchaseMockApplePaymentService_h
@import Foundation;

#include "METAInAppPurchaseApplePaymentService.h"
#include "METAInAppPurchaseMobileConfigParams.h"

@class NSMutableArray;

@interface METAInAppPurchaseMockApplePaymentService : METAInAppPurchaseApplePaymentService {
  /* instance variables */
  NSMutableArray *_transactions;
  NSMutableArray *_completedTransactions;
  METAInAppPurchaseMobileConfigParams *_mobileConfigParams;
}

/* instance methods */
- (id)initWithMobileConfigParams:(id)params;
- (void)registerTransactionObserver;
- (void)unregisterTransactionObserver;
- (void)finishTransaction:(id)transaction;
- (id)transactions;
- (void)addPayment:(id)payment;
- (void)restoreCompletedTransactions;
- (void)restoreCompletedTransactionsWithApplicationUsername:(id)username;
- (BOOL)canMakePayments;
- (id)_createTransaction:(id)transaction;
@end

#endif /* METAInAppPurchaseMockApplePaymentService_h */
