//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGAnalyticsTimeSpent_h
#define IGAnalyticsTimeSpent_h
@import Foundation;

#include "IGAnalyticsProcessingQueue.h"
#include "OS_dispatch_source-Protocol.h"

@class NSString;
@protocol IGAnalyticsTimeSpentDelegate;

@interface IGAnalyticsTimeSpent : NSObject {
  /* instance variables */
  double _lastActive;
  double _startTime;
  long long _tosArrayLengthInSeconds;
  long long _sequenceID;
  int _tosCumulativeBitsSet;
  IGAnalyticsProcessingQueue *_queue;
  double _unixTime;
  long long _persistentUptime;
  int _tosArray[2];
  NSObject<OS_dispatch_source> *_timer;
  BOOL _isActiveTimer;
  NSString *_startSessionId;
}

@property (weak, nonatomic) NSObject<IGAnalyticsTimeSpentDelegate> *delegate;

/* instance methods */
- (id)initWithQueue:(id)queue delegate:(id)delegate;
- (void)dealloc;
- (double)_currentSystemTime;
- (void)update:(double)update;
- (void)save;
- (void)reset;
- (void)_doUpdate:(double)update withUnixTime:(double)time withPersistentUptime:(long long)uptime;
- (void)_resetArray;
@end

#endif /* IGAnalyticsTimeSpent_h */
