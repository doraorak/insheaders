//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGSeparatorSupplementaryViewSource_h
#define IGSeparatorSupplementaryViewSource_h
@import Foundation;

#include "IGListCollectionContext-Protocol.h"
#include "IGListSectionController.h"
#include "IGListSupplementaryViewSource-Protocol.h"

@class NSString, UIColor;

@interface IGSeparatorSupplementaryViewSource : NSObject<IGListSupplementaryViewSource> {
  /* instance variables */
  BOOL _hasTopSeparator;
}

@property (readonly, weak, nonatomic) IGListSectionController *sectionController;
@property (readonly, weak, nonatomic) NSObject<IGListCollectionContext> *collectionContext;
@property (readonly, nonatomic) BOOL hasBottomSeparator;
@property (nonatomic) struct UIEdgeInsets { double x0; double x1; double x2; double x3; } insets;
@property (retain, nonatomic) UIColor *separatorColor;
@property (retain, nonatomic) UIColor *backgroundColor;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@optional
@property (readonly, copy) NSString *debugDescription;

/* instance methods */
- (id)initWithSectionController:(id)controller collectionContext:(id)context hasTopSeparator:(BOOL)separator hasBottomSeparator:(BOOL)separator;
- (id)supportedElementKinds;
- (id)viewForSupplementaryElementOfKind:(id)kind atIndex:(long long)index;
- (struct CGSize { double x0; double x1; })sizeForSupplementaryViewOfKind:(id)kind atIndex:(long long)index;
@end

#endif /* IGSeparatorSupplementaryViewSource_h */
