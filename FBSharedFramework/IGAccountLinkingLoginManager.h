//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGAccountLinkingLoginManager_h
#define IGAccountLinkingLoginManager_h
@import Foundation;

#include "IGAuthHeaderStore.h"
#include "IGAuthService.h"
#include "IGLoginDeferredAccountManager.h"

@protocol IGAccountLinkingLoginAnnouncer, IGUserReading;

@interface IGAccountLinkingLoginManager : NSObject {
  /* instance variables */
  NSObject<IGUserReading> *_userReader;
  IGAuthHeaderStore *_authHeaderStore;
  IGAuthService *_authService;
  IGLoginDeferredAccountManager *_deferredAccountManager;
  NSObject<IGAccountLinkingLoginAnnouncer> *_announcer;
}

/* instance methods */
- (id)initWithUserReader:(id)reader authHeaderStore:(id)store authService:(id)service loginDeferredAccountManager:(id)manager;
- (void)addListener:(id)listener;
@end

#endif /* IGAccountLinkingLoginManager_h */
