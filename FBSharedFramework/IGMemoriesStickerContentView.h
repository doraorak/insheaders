//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGMemoriesStickerContentView_h
#define IGMemoriesStickerContentView_h
@import Foundation;

#include "UIView.h"
#include "IGStaticStickerPaddedLabel.h"
#include "IGStickerViewType-Protocol.h"
#include "IGStoryStickerExportModel.h"
#include "IGUser.h"

@class NSDate, NSString;

@interface IGMemoriesStickerContentView : UIView<IGStickerViewType> {
  /* instance variables */
  IGStaticStickerPaddedLabel *_secondaryLabel;
  IGStaticStickerPaddedLabel *_primaryLabel;
  long long _stickerType;
  double _textPadding;
  IGUser *_mentionedUser;
}

@property (readonly, nonatomic) NSDate *date;
@property (readonly, nonatomic) IGStoryStickerExportModel *exportModel;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@optional
@property (readonly, copy) NSString *debugDescription;

/* class methods */
+ (id)newWithMetadata:(id)metadata resourceDirectory:(id)directory userSession:(id)session error:(id *)error;

/* instance methods */
- (id)initWithDate:(id)date mentionedUser:(id)user stickerType:(long long)type;
- (struct CGSize { double x0; double x1; })sizeThatFits:(struct CGSize { double x0; double x1; })fits;
- (void)layoutSubviews;
- (id)stickerView;
- (id)tapModelsForCompositionSize:(struct CGSize { double x0; double x1; })size;
- (id)outlinePath;
- (id)metadataForResourceDirectory:(id)directory userSession:(id)session error:(id *)error;
@end

#endif /* IGMemoriesStickerContentView_h */
