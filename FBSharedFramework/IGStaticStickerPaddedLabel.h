//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGStaticStickerPaddedLabel_h
#define IGStaticStickerPaddedLabel_h
@import Foundation;

#include "UILabel.h"

@class UIBezierPath;

@interface IGStaticStickerPaddedLabel : UILabel {
  /* instance variables */
  UIBezierPath *_cachedOutlinePath;
  BOOL _outlinePathComputationInProgress;
}

@property (nonatomic) struct UIEdgeInsets { double x0; double x1; double x2; double x3; } inset;

/* instance methods */
- (id)initWithInset:(struct UIEdgeInsets { double x0; double x1; double x2; double x3; })inset;
- (void)drawTextInRect:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })rect;
- (struct CGSize { double x0; double x1; })sizeThatFits:(struct CGSize { double x0; double x1; })fits;
- (id)outlinePath;
- (BOOL)highQualityOutlinePath;
@end

#endif /* IGStaticStickerPaddedLabel_h */
