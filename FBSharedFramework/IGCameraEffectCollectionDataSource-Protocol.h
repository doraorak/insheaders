//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGCameraEffectCollectionDataSource_Protocol_h
#define IGCameraEffectCollectionDataSource_Protocol_h
@import Foundation;

@protocol IGCameraEffectCollectionDataSource <NSObject>

@property (readonly, nonatomic) BOOL isLoading;
@property (readonly, nonatomic) NSOrderedSet *sortedEffectIdentifiers;
@property (readonly, nonatomic) NSDictionary *unsortedEffects;
@property (readonly, nonatomic) NSArray *sortedEffects;
@property (nonatomic) BOOL isAdsModeEnabled;

/* instance methods */
- (void)fetch;
- (void)prefetch;
- (void)cleanUpCache;
- (void)addListener:(id)listener;
- (void)removeListener:(id)listener;
@end

#endif /* IGCameraEffectCollectionDataSource_Protocol_h */
