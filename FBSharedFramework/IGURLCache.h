//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGURLCache_h
#define IGURLCache_h
@import Foundation;

#include "NSURLCache.h"
#include "IGCache.h"
#include "IGDiskCache.h"

@interface IGURLCache : NSURLCache {
  /* instance variables */
  id /* block */ _cacheDeletionBlock;
  IGDiskCache *_dataCache;
  IGCache *_responseCache;
}

/* instance methods */
- (id)initWithName:(id)name memoryCapacity:(unsigned long long)capacity diskCapacity:(unsigned long long)capacity maxObjectCount:(unsigned long long)count;
- (id)cachedResponseForRequest:(id)request;
- (void)storeCachedResponse:(id)response forRequest:(id)request;
- (void)removeCachedResponseForRequest:(id)request;
- (void)removeAllCachedResponses;
@end

#endif /* IGURLCache_h */
