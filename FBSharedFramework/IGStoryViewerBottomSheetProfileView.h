//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGStoryViewerBottomSheetProfileView_h
#define IGStoryViewerBottomSheetProfileView_h
@import Foundation;

#include "UIView.h"
#include "IGBouncyButton.h"
#include "IGImageView.h"

@class NSString, NSURL, UIImageView, UILabel;

@interface IGStoryViewerBottomSheetProfileView : UIView {
  /* instance variables */
  IGBouncyButton *_profileButton;
  IGImageView *_profileImageView;
  UILabel *_nameLabel;
  UIImageView *_verifiedBadge;
  UIImageView *_profilePictureAccessoryImageView;
}

@property (retain, nonatomic) NSURL *pictureURL;
@property (copy, nonatomic) NSString *name;
@property (nonatomic) BOOL verifiedBadgeVisible;

/* instance methods */
- (id)initWithFrame:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })frame;
- (struct CGSize { double x0; double x1; })sizeThatFits:(struct CGSize { double x0; double x1; })fits;
- (void)layoutSubviews;
- (void)traitCollectionDidChange:(id)change;
@end

#endif /* IGStoryViewerBottomSheetProfileView_h */
