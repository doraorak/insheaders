//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef MetaAINativeModelLoader_h
#define MetaAINativeModelLoader_h
@import Foundation;

#include "FBModelLoader.h"
#include "MetaAIModelDownloader-Protocol.h"

@class NSString;
@protocol OS_dispatch_queue;

@interface MetaAINativeModelLoader : NSObject<MetaAIModelDownloader> {
  /* instance variables */
  FBModelLoader *_modelLoader;
  NSObject<OS_dispatch_queue> *_downloadQueue;
  NSObject<OS_dispatch_queue> *_downloadCallbackQueue;
}

@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@optional
@property (readonly, copy) NSString *debugDescription;

/* instance methods */
- (id)initWithModelLoader:(id)loader;
- (void)fetchAIModels:(id)aimodels completion:(id /* block */)completion;
- (void)isModelInCache:(id)cache completion:(id /* block */)completion;
@end

#endif /* MetaAINativeModelLoader_h */
