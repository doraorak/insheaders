//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGMediaUploadDispatcher_h
#define IGMediaUploadDispatcher_h
@import Foundation;

#include "IGAPIClient-Protocol.h"
#include "IGMediaUploadDispatcherConfig.h"
#include "IGRequestPolicy.h"
#include "IGRequestToken-Protocol.h"
#include "IGURLRequest.h"

@class NSError;

@interface IGMediaUploadDispatcher : NSObject {
  /* instance variables */
  NSObject<IGAPIClient> *_dispatcher;
  NSObject<IGRequestToken> *_token;
  IGURLRequest *_request;
  IGRequestPolicy *_policy;
  IGMediaUploadDispatcherConfig *_config;
  id /* block */ _completion;
  id /* block */ _progress;
  unsigned long long _backgroundTaskIdentifier;
  long long _retryNumber;
  BOOL _cancelled;
  BOOL _executing;
  BOOL _enforceStrictCompletion;
  NSError *_lastError;
}

/* instance methods */
- (id)initWithDispatcher:(id)dispatcher enforceStrictCompletion:(BOOL)completion;
- (void)dealloc;
- (void)_startRequesting;
- (void)cancel;
- (BOOL)isExecuting;
- (void)_onNetworkReachabilityChange:(id)change;
@end

#endif /* IGMediaUploadDispatcher_h */
