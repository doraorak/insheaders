//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGSingleFeedContext_IGPlaybackCoordinator_h
#define IGSingleFeedContext_IGPlaybackCoordinator_h
@import Foundation;

#include "IGApplicationWillEnterForegroundCriticalEventsListener-Protocol.h"
#include "IGPlaybackCoordinating-Protocol.h"
#include "IGPlaybackCoordinatorStrategy-Protocol.h"

@class NSDictionary, NSMapTable, NSMutableDictionary, NSString;
@protocol IGPlaybackCoordinatorUpdateAnnouncer;

@interface IGSingleFeedContext (IGPlaybackCoordinator)
/* instance methods */
- (id)playbackCoordinator;
@end

#endif /* IGSingleFeedContext_IGPlaybackCoordinator_h */
