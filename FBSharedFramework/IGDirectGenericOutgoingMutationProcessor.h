//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef IGDirectGenericOutgoingMutationProcessor_h
#define IGDirectGenericOutgoingMutationProcessor_h
@import Foundation;

#include "IGDirectMutationNetworker.h"
#include "IGDirectMutationNetworkerDataProvider.h"
#include "IGDirectMutationRequestProcessing-Protocol.h"

@class NSString;

@interface IGDirectGenericOutgoingMutationProcessor : NSObject<IGDirectMutationRequestProcessing> {
  /* instance variables */
  IGDirectMutationNetworker *_networker;
  IGDirectMutationNetworkerDataProvider *_dataProvider;
}

@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@optional
@property (readonly, copy) NSString *debugDescription;

/* instance methods */
- (id)initWithNetworker:(id)networker dataProvider:(id)provider;
- (void)executeWithResultHandler:(id)handler accessoryPackage:(id)package;
- (void)handleMutationStateChange:(id)change;
@end

#endif /* IGDirectGenericOutgoingMutationProcessor_h */
