//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.468, BuildCommit: a1243d3a18f9b0e29af55a12ad74ccd2cd1f0abc)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 15.0, SDK: 17.4, Tool: ld (16.0)
//    - LC_SOURCE_VERSION: 
//
#ifndef _TtC13IGUDMStickers9TrackData_h
#define _TtC13IGUDMStickers9TrackData_h
@import Foundation;

#include "_TtCs12_SwiftObject.h"

@interface _TtC13IGUDMStickers9TrackData : _TtCs12_SwiftObject { // (Swift)
  /* instance variables */
   allowsSaving;
   artistId;
   audioAssetId;
   audioClusterId;
   coverArtworkThumbnailUri;
   coverArtworkUri;
   darkMessage;
   dashManifest;
   displayArtist;
   durationInMs;
   fastStartProgressiveDownloadUrl;
   hasLyrics;
   highlightStartTimesInMs;
   id;
   igUsername;
   isEligibleForAudioEffects;
   isExplicit;
   progressiveDownloadUrl;
   reactiveAudioDownloadUrl;
   sanitizedTitle;
   subtitle;
   title;
   web30sPreviewDownloadUrl;
}

@end

#endif /* _TtC13IGUDMStickers9TrackData_h */
